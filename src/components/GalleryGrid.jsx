...
export default function GalleryGrid({ images, category }) {
  ...
  const openModal = useCallback((img, i) => setSelected({ ...img, index: i }), [])
  const handlePrev = useCallback(() => {
    if (!selected) return
    const i = (selected.index - 1 + images.length) % images.length
    setSelected({ ...images[i], index: i })
  }, [selected, images])
  const handleNext = useCallback(() => {
    if (!selected) return
    const i = (selected.index + 1) % images.length
    setSelected({ ...images[i], index: i })
  }, [selected, images])
  ...
  return (
    <>
      <div className="grid gap-3" style={{ gridTemplateColumns:'repeat(auto-fill, minmax(220px, 1fr))', gridAutoRows:'200px' }}>
        {images.map((img, i) => (
          <motion.div
            ...
            className={`relative overflow-hidden cursor-zoom-in group ${getGridClass(i)}`}
          ...
          >
            ...
            {/* Overlay info */}
            <div className="absolute inset-0 pointer-events-none transition-opacity duration-500" onClick={() => openModal(img, i)}
      ...
      <AnimatePresence>
        {selected && (
          <ImageModal image={selected} onClose={() => setSelected(null)} onPrev={handlePrev} onNext={handleNext} total={images.length} category={category} />
        )}
      </AnimatePresence>
    </>
  )
